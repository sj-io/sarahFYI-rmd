...
MK8D_run2csv ./_data/splits/trks_48.csv ./_data/splits MK8D_runs_med.csv median
```
...
ggMax <- r0 %>% 
  ...
  scale_alpha_continuous(range = c(.35, .9)) +
  labs(x = "",
...
c2 <- cups %>% 
  ...
  ungroup() %>% 
  select(runID) %>% 
  left_join(cups, by = "runID") %>% 
  select(cup, track, cumtime) %>% 
  rename(max = cumtime)

# Add max run to respective cup
c3 <- cups %>% 
  left_join(c2, by = c("cup", "track")) %>% 
  group_by(track) %>% 
  mutate(mean = mean(cumtime),
         median = median(cumtime)) %>% 
  ungroup() %>% 
  pivot_longer(cols = max:median, names_to = "stat", values_to = "ctime") %>% 
  mutate(
