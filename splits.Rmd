...
# Best possible time & labels
sob48 <- min48 %>% 
  mutate(Time = cumsum(PBtime),
         Time = seconds(Time)) %>% 
  select(Track, Time, PBtime, PBdate) %>% 
  ...
  mutate(offset = Time - c_time,
         label = paste0("<b>Best Possible Time</b>", 
                        ...
                        "<br><b>Run Time:</b> ",
                        round(seconds_to_period(Time), 2)
                        )) %>% 
  select(-c(PBtime, PBdate))
...
max48 <- r0 %>%
  ...
  mutate(label = paste0("<b>", "Run #", runNO, " ", date, "</b>", 
                        "<br><b>Track:</b> ", Track,
                        "<br><b>Split Time:</b> ", round(seconds_to_period(Time.y), 2),
                        "<br><b>Run Time:</b> ", round(seconds_to_period(Time.x), 2)
                        ),
         label = case_when(
           ptss == 0 ~ label,
           is.na(ptss) ~ label,
