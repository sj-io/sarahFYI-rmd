...
description: |
  Reviewing my stream snipe of Clint Stevens in Mario Kart.
author:
  - name: Sarah Johnson
categories:
  - online
...
<aside>![](images/phoenix-sweat.jpg){width="212"} Sorry for spamming "ADD SARAH" in chat when you were trying to give an update on Juno. Thanks for the add.</aside>
...
[^1]: If anyone else wants to help collect this data, please let me know!
...
ggSNP <- snp %>% ggplot(aes(x = race, y = place, color = racer)) +
  geom_line() +
...
ggSmooth <- snp %>% ggplot(aes(x = race, y = place, color = racer)) +
  geom_smooth() +
  scale_y_reverse() +
  theme_minimal()
...
Clint's strong start turned into a noticeable lull, followed by a momentum swing which died out towards the end. His overall average was between 6th and 7th place. My results were mostly stagnant, averaging between 8th and 9th place.
...
<aside>![](images/chat-reaction-01.png){width="172"}</aside>
...
The unfortunate part of analyzing this data is clearly seeing that I'm not very good compared to the others. But that's okay! I know I can get better. The main reasons I suck at the moment are:
...
But I refuse to change my build.
