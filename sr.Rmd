...
abr <- read_csv("_data/track-abbr.csv") %>% 
  pivot_longer(cols = c(trk, split, track), 
               names_to = "ref", values_to = "og") %>% 
  select(trkNO, og, lss, cup, type) %>% 
  distinct() %>% 
  add_row(trkNO = 0, og = "Start", lss = "Start", 
          cup = " ", type = " ", .before = 1) %>% 
...
runs <- clean %>% 
  ...
  left_join(PB_tracks, by = c("category", "cup", "track"), copy = TRUE) %>% 
  filter(!is.na(runNO)) %>% 
  group_by(ID) %>% 
  arrange(track) %>% 
...
BPT <- rbind(PB_48, PB_cups) %>% 
  mutate(time = PBtime, date = PBdate, runNO = 0, 
...
runs_full <- runs %>% 
  ...
  mutate(label = paste0("<b>", "Run #", runNO, " ", date, "</b>", 
                        ...
                        "<br><b>Run Time:</b> ", round(seconds_to_period(cumsum), 2))) %>% 
  rbind(BPT) %>% 
  left_join(slowest_runs, by = c("category", "cup", "track")) %>% 
  left_join(PB_runs, by = c("ID", "track")) %>% 
...
run_time <- runs %>% 
  ...
  slice_max(track) %>% 
  ungroup() %>% 
  select(runNO, category, cup, date, cumsum) %>% 
  mutate(cup = factor(cup, levels = lvl))
```
...
run_time_48 <- run_time %>% 
  filter(category == "48 Tracks") %>% 
  mutate(label = paste0("<b>Run #", runNO, " ", date, "</b>",
                        "<br>", round(seconds_to_period(cumsum), 2))) %>% 
...
traces_nitro <- ggplot() +
  ...
  scale_y_time() +
  scale_size_continuous(range = c(.05, 2.25)) +
  scale_color_identity() +
  scale_alpha_continuous(range = c(.35, .8)) +
  labs(x = "",
       y = "") +
...
ggplotly(run_time_n, tooltip = "text")
```
...
runs_retro <- runs_retro %>% 
  mutate(clr = ifelse(runNO == 0, "grey", colors_retro[runs_retro$runNO]))
```
...
ggplotly(run_time_r, tooltip = "text")
```
...
ggplotly(run_time_b, tooltip = "text")
```
...
ggplotly(run_time_d, tooltip = "text")
```
